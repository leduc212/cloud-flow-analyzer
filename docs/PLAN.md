...
- ✅ Demo site on GitHub Pages (`apps/web`, deployed from `main` by the Site workflow): sample flows, paste or open a flow definition, analysed in the browser. The repo is public (2026-09-25).

### v1.0: Polish and publish
- ✅ Docs page per rule: the site's rules page (one anchor per rule, generated from the rule catalogue); every finding in the pane links to it, and docs links carry readable titles.
- ✅ README with screenshots (`docs/images`, regenerated by `SCREENSHOTS=1 pnpm test:e2e screenshots`). A GIF was left out: screenshots show the same and stay reproducible.
- ✅ Accessibility check: axe (WCAG 2.1 A/AA) in the browser tests on the pane, popup, All flows page, capture tool and the site in light and dark; serious or critical problems fail CI. One found and fixed (an unlabelled select).
- ✅ Security review: the worker checks each message's sender (`MESSAGE_SOURCES`); no `innerHTML`/`eval`; SECURITY.md (reporting, token handling, permissions, stored data); CodeQL on `main`, pull requests and weekly.
- ✅ Privacy policy on the site (and PRIVACY.md).
- ✅ Version 1.0.0, CHANGELOG.md (release notes come from it), store listing text in `docs/STORE.md`.
- ⏳ Owner: enable Pages (Source: GitHub Actions) and private vulnerability reporting, merge to `main`, tag `v1.0.0`, submit to Edge Add-ons (Chrome Web Store if decided) with real-portal screenshots.
...
9. Repo goes public at v0.3 (needed for free Pages and CodeQL). Done 2026-09-25.
